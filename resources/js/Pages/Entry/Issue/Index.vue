...
<!--            </a>-->
        ...
        <Table>
            ...
            <template v-slot:tbody>
                <tr v-if="issues.data.length > 0" v-for="issue in issues.data" :key="issue.id">
                    <td>{{ issue.id }}</td>
                    <td>{{ issue.name }}</td>
                    ...
                    <td class="">
                        <div class="">
                            <Button @click="handleDelete(issue.id)" text="" icon="fa-fw fa-trash" btn-type="me-2 btn-sm btn-outline-danger"/>
                            <Button @click="handleEdit(issue.id)" text="" icon="fa-fw fa-pen" btn-type="btn-sm me-2 btn-outline-warning"/>
                            <Button @click="handleShow(issue.id)" text="" icon="fa-fw fa-circle-info" btn-type="btn-sm btn-outline-info"/>
                        </div>
                    </td>
                    <td>
                        <div class="">
                            <div class="">
                                <small>
                                    ...
                                    {{ dataFormat(issue.created_at, 'll') }}
                                </small>
                            </div>
                            <div class="">
                                <small>
                                    <i class="fa-solid fa-clock text-primary"></i>
                                    {{ dataFormat(issue.created_at, 'LT') }}
                                </small>
                            </div>
                        </div>
                    </td>
        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="">
                <p class="fw-bolder mb-0">
                    Total : <span class="text-primary">{{ issues.total }}</span>
                </p>
            </div>
        </div>
...
export default {
    ...
    layout : Layout,
    props : ['issues', 'imgPath', 'status', 'filteredValue', 'issues'],
    data() {
        return {
            isDefault: '',
            params: {
    ...
    methods: {
        ...
        handleEdit(id){
            Inertia.get(route('issue.edit',id))
        },
        handleCreate(){
            Inertia.get(route('issue.create'))
        },
        handlePdf(){
            Inertia.get(route('issue.exportPdf'))
        },
        importCSV(){
            Inertia.post(route('issue.importCSV'),this.form2,{
                forceFormData: true,
                // onSuccess: () => {
                //     console.log("created");
                //     const Toast = this.$swal.mixin({
        ...
        handleShow(id){
            Inertia.get(route('issue.show', id));
        }
