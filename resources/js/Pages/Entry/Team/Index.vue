...
<!--            </a>-->
            ...
            <Button @click="handleCreate" :text="$t('team_add_btn')" href="team.create" icon="fa-plus" btn-type="btn-primary"/>
        </div>
        ...
        <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                        <a :href="route('team.exportCSV',this.form.extension)">
        ...
        <Table>
            ...
            <template v-slot:tbody>
                <tr v-if="teams.data.length > 0" v-for="team in teams.data" :key="team.id">
                    <td>{{ team.id }}</td>
                    <td>{{ team.name }}</td>

                    <td>
                        <div class="form-check form-switch">
                            <input @change="handleStatus(team.id)" :checked=team.status class="form-check-input" type="checkbox" role="switch" >
                        </div>
                    </td>
                    <td class="">
                        <div class="">
                            <Button @click="handleDelete(team.id)" text="" icon="fa-fw fa-trash" btn-type="me-2 btn-sm btn-outline-danger"/>
                            <Button @click="handleEdit(team.id)" text="" icon="fa-fw fa-pen" btn-type="btn-sm btn-outline-warning"/>
                        </div>
                    </td>
                    <td>
                        <div class="">
                            <div class="">
                                <small>
                                    ...
                                    {{ dataFormat(team.created_at, 'll') }}
                                </small>
                            </div>
                            <div class="">
                                <small>
                                    <i class="fa-solid fa-clock text-primary"></i>
                                    {{ dataFormat(team.created_at, 'LT') }}
                                </small>
                            </div>
                        </div>
                    </td>
        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="">
                <p class="fw-bolder mb-0">
                    Total : <span class="text-primary">{{ teams.total }}</span>
                </p>
            </div>
        </div>
...
export default {
    ...
    layout : Layout,
    props : ['teams', 'imgPath', 'status', 'filteredValue'],
    data() {
        return {
            isDefault: '',
            params: {
    ...
    watch: {
        params: {
            handler : debounce(function (val){
                console.log(val);
                    Inertia.get(route('team.index'), this.params, {
                        replace : true,
                        preserveState: true
                    })
    ...
    methods: {
        ...
        handleDefault(id){
            Inertia.put(route('team.default',id));
        },
        handleStatus(id){
            Inertia.put(route('team.status',id),'',{
                onSuccess: () => {
                    console.log("created");
        ...
        handleEdit(id){
            Inertia.get(route('team.edit',id))
        },
        handleCreate(){
            Inertia.get(route('team.create'))
        },
        handlePdf(){
            Inertia.get(route('team.exportPdf'))
        },
        importCSV(){
            Inertia.post(route('team.importCSV'),this.form2,{
                forceFormData: true,
                // onSuccess: () => {
                //     console.log("created");
                //     const Toast = this.$swal.mixin({
